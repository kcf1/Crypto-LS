...
st.caption(
    "5m data aggregated to hourly bars, signal = (price - mid) / range × 2, smoothed, with Weibull CDF vol tilt "
...
with col_controls:
    ...
    breakout_window1 = st.slider("Breakout window (hours)", 24, 500, 48, key="breakout1_bv")
    smooth_window1 = st.slider("Smooth window (hours)", 1, 48, 12, key="smooth1_bv")
    vol_look1 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol1_bv")
    weibull_c1 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c1_bv")
    alpha_blend1 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha1_bv")
    ...
    breakout_window2 = st.slider("Breakout window (hours)", 24, 500, 96, key="breakout2_bv")
    smooth_window2 = st.slider("Smooth window (hours)", 1, 48, 12, key="smooth2_bv")
    vol_look2 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol2_bv")
    weibull_c2 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c2_bv")
    alpha_blend2 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha2_bv")
    ...
    breakout_window3 = st.slider("Breakout window (hours)", 24, 500, 192, key="breakout3_bv")
    smooth_window3 = st.slider("Smooth window (hours)", 1, 48, 12, key="smooth3_bv")
    vol_look3 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol3_bv")
    weibull_c3 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c3_bv")
    alpha_blend3 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha3_bv")
    ...
    st.divider()
    target_vol = st.slider("Target vol (ann) — all series", 0.05, 0.50, 0.30, 0.01, key="target_vol_bv")
...
with col_main:
    ...
    st.caption("Signal: (price - mid) / range × 2, smoothed, × Weibull CDF vol tilt, × strategy decay (optional), vol-matched × cap.")
