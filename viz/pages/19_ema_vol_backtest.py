"""
Page: EMA Vol strategy backtest on EOD data.

Signal = EMA crossover with Weibull CDF vol tilt and strategy decay. Classic trend-following
modulated by volatility regime. Reduces size in high-vol regimes via Weibull CDF.
...
with col_controls:
    ...
    show_s1 = st.checkbox("Show Strategy 1", value=True, key="show_s1_ev")
    show_s2 = st.checkbox("Show Strategy 2", value=True, key="show_s2_ev")
    show_s3 = st.checkbox("Show Strategy 3", value=True, key="show_s3_ev")
    show_avg = st.checkbox("Show Avg (1+2+3)", value=True, key="show_avg_ev")
    show_reg = st.checkbox("Show Reg (β≥0)", value=True, key="show_reg_ev")
...
def run_strategy(
    ...
    """
    EMA Vol strategy with Weibull CDF vol tilt and strategy decay.
    ...
    signal_raw = fast_ema_series - slow_ema_series
    ...
    # 3. Volatility: Rogers-Satchell (or EWM std as fallback)
    try:
...
pnl1, pos1, s1 = run_strategy(fast1, slow_mult1, std_look1, vol_look1, weibull_c1, alpha_blend1, use_decay1, target_vol, cap1)
pnl2, pos2, s2 = run_strategy(fast2, slow_mult2, std_look2, vol_look2, weibull_c2, alpha_blend2, use_decay2, target_vol, cap2)
pnl3, pos3, s3 = run_strategy(fast3, slow_mult3, std_look3, vol_look3, weibull_c3, alpha_blend3, use_decay3, target_vol, cap3)
...
fig.update_layout(
    title=f"{symbol} Cumulative log return (EMA Vol with Weibull tilt)",
    xaxis_title="Date",
