"""
Page: Acceleration (momentum-of-momentum) backtest on hourly data.

Signal = acceleration of EMA crossover (diff of standardized level) rather than level itself.
Captures trend inflection and strength changes. With Weibull CDF vol tilt and strategy decay.
...
st.title("Acceleration (momentum-of-momentum) backtest")
st.caption(
    "5m data aggregated to hourly bars, signal = acceleration of EMA crossover (diff of standardized level), "
...
with col_controls:
    ...
    fast1 = st.slider("Fast EMA (hours)", 12, 300, 24, key="fast1_acc")
    slow_mult1 = st.slider("Slow multiple", 2, 8, 2, key="slow_mult1_acc")
    diff_mult1 = st.slider("Diff multiplier", 0.5, 3.0, 1.0, 0.1, key="diff_mult1_acc")
    std_look1 = st.slider("Std lookback (hours)", 24, 720, 720, key="std1_acc")
    vol_look1 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol1_acc")
    weibull_c1 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c1_acc")
    alpha_blend1 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha1_acc")
    use_decay1 = st.checkbox("Use strategy decay", value=True, key="decay1_acc")
    cap1 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap1_acc")

    st.subheader("Strategy 2")
    fast2 = st.slider("Fast EMA (hours)", 12, 300, 48, key="fast2_acc")
    slow_mult2 = st.slider("Slow multiple", 2, 8, 2, key="slow_mult2_acc")
    diff_mult2 = st.slider("Diff multiplier", 0.5, 3.0, 1.0, 0.1, key="diff_mult2_acc")
    std_look2 = st.slider("Std lookback (hours)", 24, 720, 720, key="std2_acc")
    vol_look2 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol2_acc")
    weibull_c2 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c2_acc")
    alpha_blend2 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha2_acc")
    use_decay2 = st.checkbox("Use strategy decay", value=True, key="decay2_acc")
    cap2 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap2_acc")

    st.subheader("Strategy 3")
    fast3 = st.slider("Fast EMA (hours)", 12, 300, 96, key="fast3_acc")
    slow_mult3 = st.slider("Slow multiple", 2, 8, 2, key="slow_mult3_acc")
    diff_mult3 = st.slider("Diff multiplier", 0.5, 3.0, 1.0, 0.1, key="diff_mult3_acc")
    std_look3 = st.slider("Std lookback (hours)", 24, 720, 720, key="std3_acc")
    vol_look3 = st.slider("Vol window (hours)", 24, 1440, 720, key="vol3_acc")
    weibull_c3 = st.slider("Weibull shape (c)", 0.5, 3.0, 2.0, 0.1, key="weibull_c3_acc")
    alpha_blend3 = st.slider("Alpha blend", 0.0, 1.0, 1.0, 0.1, key="alpha3_acc")
    use_decay3 = st.checkbox("Use strategy decay", value=True, key="decay3_acc")
    cap3 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap3_acc")

    st.divider()
    target_vol = st.slider("Target vol (ann) — all series", 0.05, 0.50, 0.30, 0.01, key="target_vol_acc")

    st.divider()
    show_raw = st.checkbox("Show Raw", value=True, key="show_raw_acc")
    show_s1 = st.checkbox("Show Strategy 1", value=True, key="show_s1_acc")
    show_s2 = st.checkbox("Show Strategy 2", value=True, key="show_s2_acc")
    show_s3 = st.checkbox("Show Strategy 3", value=True, key="show_s3_acc")
    show_avg = st.checkbox("Show Avg (1+2+3)", value=True, key="show_avg_acc")
    show_reg = st.checkbox("Show Reg (β≥0)", value=True, key="show_reg_acc")
...
def run_strategy(
    ...
    """
    Acceleration strategy: uses acceleration of EMA crossover rather than level.
    With Weibull CDF vol tilt and strategy decay.
    
    Returns: (pnl, position, signal_accel)
    """
    # 1. Calculate EMA crossover level
    ...
    signal_level_raw = fast_ema_series - slow_ema_series
    
    # 2. Standardize the level
    ...
    # 3. Calculate acceleration (diff of standardized level)
    diff_window = int(fast_ema * diff_multiplier)
    signal_accel_raw = signal_level.diff(diff_window)
    
    # 4. Re-standardize acceleration
    ...
    return pnl, position, signal_accel


pnl1, pos1, s1 = run_strategy(fast1, slow_mult1, diff_mult1, std_look1, vol_look1, weibull_c1, alpha_blend1, use_decay1, target_vol, cap1)
...
fig.update_layout(
    title=f"{symbol} Cumulative log return (Acceleration with Weibull tilt)",
    xaxis_title="Date",
