"""
Page: Acceleration (momentum-of-momentum) backtest on EOD data.

Signal = acceleration of EMA crossover (diff of standardized level) rather than level itself.
Captures trend inflection and strength changes. Standardized and vol-matched position.
...
st.caption(
    "EOD close, signal = acceleration of EMA crossover (diff of standardized level), "
    "standardized and capped; vol-matched position. Cum return = cumsum(log return)."
)
...
with st.sidebar:
    ...
    fast1 = st.slider("Fast lookback (days)", 1, 30, 20, key="fast1_acc")
    slow_mult1 = st.slider("Slow multiple", 2, 8, 4, key="slow_mult1_acc")
    diff_mult1 = st.slider("Diff multiplier", 1, 5, 2, key="diff_mult1_acc")
    std_look1 = st.slider("Std lookback (days)", 5, 90, 30, key="std1_acc")
    vol_look1 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol1_acc")
    cap1 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap1_acc")

    st.subheader("Strategy 2")
    fast2 = st.slider("Fast lookback (days)", 1, 30, 20, key="fast2_acc")
    slow_mult2 = st.slider("Slow multiple", 2, 8, 4, key="slow_mult2_acc")
    diff_mult2 = st.slider("Diff multiplier", 1, 5, 2, key="diff_mult2_acc")
    std_look2 = st.slider("Std lookback (days)", 5, 90, 30, key="std2_acc")
    vol_look2 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol2_acc")
    cap2 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap2_acc")

    st.subheader("Strategy 3")
    fast3 = st.slider("Fast lookback (days)", 1, 30, 20, key="fast3_acc")
    slow_mult3 = st.slider("Slow multiple", 2, 8, 4, key="slow_mult3_acc")
    diff_mult3 = st.slider("Diff multiplier", 1, 5, 2, key="diff_mult3_acc")
    std_look3 = st.slider("Std lookback (days)", 5, 90, 30, key="std3_acc")
    vol_look3 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol3_acc")
    cap3 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap3_acc")

    st.divider()
    target_vol = st.slider("Target vol (ann) — all series", 0.05, 0.50, 0.15, 0.01, key="target_vol_acc")
...
def run_strategy(
    ...
    signal_level_raw = fast_ema - slow_ema
    ...
    signal_level = (signal_level_raw / signal_level_std_series).clip(-3, 3)
    
    # 3. Calculate acceleration (diff of standardized level)
    diff_window = fast_lookback * diff_multiplier
    ...
    # 5. Volatility estimation and position sizing
    ...
    position = (signal_accel * target_daily / vol_estimator).clip(-cap, cap)
    
    # 6. PnL calculation (shift position by 1 to avoid lookahead)
...
pnl1, pos1, s1 = run_strategy(fast1, slow_mult1, diff_mult1, std_look1, vol_look1, target_vol, cap1)
pnl2, pos2, s2 = run_strategy(fast2, slow_mult2, diff_mult2, std_look2, vol_look2, target_vol, cap2)
pnl3, pos3, s3 = run_strategy(fast3, slow_mult3, diff_mult3, std_look3, vol_look3, target_vol, cap3)
...
fig.update_layout(
    title=f"{symbol} Cumulative log return (acceleration backtest)",
    xaxis_title="Date",
...
# Stats (enhanced with all metrics from SOP)
...
def build_stats(ret_series: pd.Series, is_raw: bool, trading_days: int = 252) -> dict:
    """Enhanced build_stats with all metrics."""
    if is_raw:
        r = ret_series
...
st.caption("Signal: acceleration of EMA crossover (diff of standardized level), re-standardized, vol-matched × cap.")
