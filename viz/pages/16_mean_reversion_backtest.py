...
st.caption(
    "EOD close, signal = -sign(momentum) when |momentum| > threshold and volume decay > threshold, "
...
with st.sidebar:
    ...
    vol_look1 = st.slider("Vol window (days)", 5, 90, 30, key="vol1_mr")
    reversal_look1 = st.slider("Reversal window (days)", 1, 20, 5, key="rev1_mr")
    reversal_thresh1 = st.slider("Reversal threshold", 0.5, 3.0, 1.5, 0.1, key="rev_thresh1_mr")
    volume_thresh1 = st.slider("Volume threshold", 0.1, 0.9, 0.5, 0.05, key="vol_thresh1_mr")
    cap1 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap1_mr")

    st.subheader("Strategy 2")
    vol_look2 = st.slider("Vol window (days)", 5, 90, 30, key="vol2_mr")
    reversal_look2 = st.slider("Reversal window (days)", 1, 20, 5, key="rev2_mr")
    reversal_thresh2 = st.slider("Reversal threshold", 0.5, 3.0, 1.5, 0.1, key="rev_thresh2_mr")
    volume_thresh2 = st.slider("Volume threshold", 0.1, 0.9, 0.5, 0.05, key="vol_thresh2_mr")
    cap2 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap2_mr")

    st.subheader("Strategy 3")
    vol_look3 = st.slider("Vol window (days)", 5, 90, 30, key="vol3_mr")
    reversal_look3 = st.slider("Reversal window (days)", 1, 20, 5, key="rev3_mr")
    reversal_thresh3 = st.slider("Reversal threshold", 0.5, 3.0, 1.5, 0.1, key="rev_thresh3_mr")
    volume_thresh3 = st.slider("Volume threshold", 0.1, 0.9, 0.5, 0.05, key="vol_thresh3_mr")
    cap3 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap3_mr")

    st.divider()
    target_vol = st.slider("Target vol (ann) — all series", 0.05, 0.50, 0.15, 0.01, key="target_vol_mr")
...
# Aggregate to EOD: last close and sum volume
eod = df.groupby("date", as_index=False).agg(close=("close", "last"), volume=("volume", "sum"))
...
def run_strategy(
    ...
    vol_annualized = vol_estimator * np.sqrt(TRADING_DAYS)
    ...
    position = (signal * target_daily / vol_annualized).clip(-cap, cap)
...
# Stats (copy from block_momentum - same helper functions)
