"""
Page: Trend-following backtest with volatility-regime scaler.

Same as trend-following backtest, plus position scaled by volatility regime:
regime_scaler = (rolling_1pct_vol / current_vol).clip(0.5, 2.0) with configurable lookback.
"""
...
st.caption(
    "EOD close, trend signal, vol-matched position; position scaled by volatility regime "
    "(rolling 1-percentile vol / current vol, clipped [0.5, 2])."
)
...
with st.sidebar:
    ...
    std_look1 = st.slider("Std lookback (days)", 5, 90, 30, key="std1_reg")
    vol_look1 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol1_reg")
    cap1 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap1_reg")

    st.subheader("Strategy 2")
    fast2 = st.slider("Fast lookback (days)", 1, 30, 20, key="fast2_reg")
    slow_mult2 = st.slider("Slow multiple", 2, 8, 4, key="slow_mult2_reg")
    std_look2 = st.slider("Std lookback (days)", 5, 90, 30, key="std2_reg")
    vol_look2 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol2_reg")
    cap2 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap2_reg")

    st.subheader("Strategy 3")
    fast3 = st.slider("Fast lookback (days)", 1, 30, 20, key="fast3_reg")
    slow_mult3 = st.slider("Slow multiple", 2, 8, 4, key="slow_mult3_reg")
    std_look3 = st.slider("Std lookback (days)", 5, 90, 30, key="std3_reg")
    vol_look3 = st.slider("Vol-estimator lookback (days)", 5, 90, 30, key="vol3_reg")
    cap3 = st.slider("Position cap", 0.5, 5.0, 3.0, 0.1, key="cap3_reg")

    st.divider()
    st.subheader("Volatility regime")
    regime_lookback = st.slider(
        "Rolling 1-percentile lookback (days)",
        21,
        252,
        63,
        help="Lookback for rolling 1st percentile of daily vol; position scaled by (1pct_vol/current_vol) clipped [0.5, 2].",
        key="regime_lookback",
    )

    st.divider()
    target_vol = st.slider("Target vol (ann) — all series", 0.05, 0.50, 0.15, 0.01, key="target_vol_reg")

    st.divider()
    show_raw = st.checkbox("Show Raw", value=True, key="show_raw_reg")
    show_s1 = st.checkbox("Show Strategy 1", value=True, key="show_s1_reg")
    show_s2 = st.checkbox("Show Strategy 2", value=True, key="show_s2_reg")
    show_s3 = st.checkbox("Show Strategy 3", value=True, key="show_s3_reg")
    show_avg = st.checkbox("Show Avg (1+2+3)", value=True, key="show_avg_reg")
    show_reg = st.checkbox("Show Reg (β≥0)", value=True, key="show_reg_reg")
...
def run_strategy(
    ...
    cap: float,
    regime_lookback_days: int | None = None,
    ...
    if regime_lookback_days is not None:
        vol_series = ret.ewm(span=vol_lookback, adjust=False).std().clip(lower=VOL_FLOOR)
        rolling_1pct_vol = vol_series.rolling(window=regime_lookback_days).quantile(0.01)
        regime_scaler = (rolling_1pct_vol / vol_series).clip(0.5, 2.0)
        regime_scaler = regime_scaler.fillna(1.0)
        position = position * regime_scaler
...
pnl1, pos1, s1 = run_strategy(fast1, slow_mult1, std_look1, vol_look1, target_vol, cap1, regime_lookback)
...
fig.update_layout(
    title=f"{symbol} Cumulative log return (trend-following + regime)",
    xaxis_title="Date",
...
st.caption(f"Volatility regime: rolling 1pct lookback = {regime_lookback} days; position × (1pct_vol/vol).clip(0.5, 2).")
