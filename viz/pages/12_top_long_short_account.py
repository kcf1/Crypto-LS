"""
Page: Plot Top Trader Long/Short Account Ratio.
Visualize long_short_ratio, long_account, short_account (top 20% traders by margin). 5m period.
"""
...
st.title("Top Trader Long/Short Account Ratio")
st.caption("Plot top 20% traders (by margin) long/short account ratio. 5m period; Binance retains ~30 days.")
...
rows = storage.read_top_long_short_account(symbol=symbol, period=PERIOD, start_time=start_time_ms, end_time=end_time_ms)

if not rows:
    st.warning(f"No top long/short account data for **{symbol}** ({PERIOD}) in the last {days_back} days. Run test_backfill_top_long_short_account.py first.")
    st.stop()
...
fig.update_layout(height=500, showlegend=True, hovermode="x unified", template="plotly_white", xaxis_title="Time (UTC)", yaxis_title="Ratio / %", title=f"{symbol} Top Trader L/S Account (last {days_back} days)")
