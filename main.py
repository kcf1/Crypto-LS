...
def main() -> None:
    ...
    _ = settings
    _ = secrets
    # Data: collector (raw ingest), storage (raw OHLCV). Uses DATABASE_URL when set, else SQLite.
    storage = Storage()
    ...
    # Booking: ledger for orders/trades/positions. Uses DATABASE_URL when set, else SQLite.
    ledger = Ledger()
