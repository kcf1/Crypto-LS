"""
Check for missing top long/short account records.
Updates every 5 minutes (5m period).

Run from project root: python scripts/integrity/check_missing_top_long_short_account.py
"""
...
def check_symbol(storage: Storage, symbol: str, period: str, start_time: datetime, end_time: datetime) -> dict:
    ...
    rows = storage.read_top_long_short_account(
        symbol=symbol,
        period=period,
...
def save_results(reports: list, summary: dict, start_time: datetime, end_time: datetime, output_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = output_dir / f"missing_top_long_short_account_{timestamp}.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(json_data, f, indent=2, ensure_ascii=False)

    txt_path = output_dir / f"missing_top_long_short_account_{timestamp}.txt"
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("TOP LONG/SHORT ACCOUNT DATA INTEGRITY CHECK REPORT\n")
        f.write("=" * 80 + "\n\n")
...
def main() -> int:
    ...
    print(f"Checking missing top long/short account records for last {DAYS_BACK} days (UTC)")
