"""
Check for missing basis records.
Basis updates every 5 minutes (5m period).

Run from project root: python scripts/integrity/check_missing_basis.py
"""
...
def save_results(reports: list, summary: dict, start_time: datetime, end_time: datetime, output_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = output_dir / f"missing_basis_{timestamp}.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(json_data, f, indent=2, ensure_ascii=False)

    txt_path = output_dir / f"missing_basis_{timestamp}.txt"
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("BASIS DATA INTEGRITY CHECK REPORT\n")
        f.write("=" * 80 + "\n\n")
