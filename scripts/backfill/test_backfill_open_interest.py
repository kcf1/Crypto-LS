"""
Test script: Backfill Open Interest for first 10 symbols only.
Quick test to verify open interest collection works before running full backfill.

Run from project root: python scripts/backfill/test_backfill_open_interest.py
"""
...
# Test with shorter time range: last 7 days (Binance retains ~30 days)
DAYS_BACK = 7
MS_PER_7D = int(DAYS_BACK * 24 * 3600 * 1000)

DELAY_SEC = 0.2
MAX_429_RETRIES = 5
MAX_EMPTY_RETRIES = 3  # Retry if empty response (likely rate limit for 7-day range)
EMPTY_RETRY_DELAY = 0.5  # Reduced delay: 0.5s instead of 2.0s
...
def fetch_open_interest_with_retry(
    ...
    *,
    ...
    Also retries on empty responses (likely rate limiting for 7-day range) up to MAX_EMPTY_RETRIES.
    
    Common failure reasons:
    - HTTP 429: Rate limit exceeded (1000 req/5min IP limit)
    - HTTP 418: IP banned (2min-3days, escalates on repeat offenses)
    - Empty response: Likely rate limit (for 7-day range, gaps unlikely)
    ...
    for attempt in range(MAX_429_RETRIES):
        try:
            ...
            # Retry if empty response (likely rate limit for 7-day range)
...
def main() -> None:
    ...
    logger.info("TEST: Open interest backfill for %s symbols, period=%s, last %s days", len(symbols), PERIOD, DAYS_BACK)
    ...
    # Calculate estimated total records per symbol (7 days * 24 hours * 12 intervals/hour)
    # This is an estimate - actual may vary, but we'll update the total dynamically
    estimated_records_per_symbol = DAYS_BACK * 24 * 12  # 5-minute intervals (7 days = ~2016 records)

    for i, symbol in enumerate(symbols):
        logger.info("Symbol %s/%s: %s", i + 1, len(symbols), symbol)
        symbol_rows = 0
        
        # Create progress bar for this symbol
        pbar = tqdm(
            total=estimated_records_per_symbol,
            desc=f"[{i+1}/{len(symbols)}] {symbol}",
            unit="records",
            ncols=120,
        ...
        try:
            ...
            while current_end_time > start_time_ms:
                ...
                if filtered_rows:
                    ...
                    # Update progress bar
                    pbar.update(len(filtered_rows))
                    
                    # Adjust total if we've exceeded the estimate
                    if symbol_rows > pbar.total:
                        pbar.total = symbol_rows + LIMIT  # Add buffer for next batch
            ...
            # Set final total to actual count
            pbar.total = symbol_rows
            pbar.refresh()
            pbar.close()
            ...
            print(f"  [{i+1}/{len(symbols)}] {symbol}: {symbol_rows} records (expected: ~{expected_rows}, coverage: {coverage_pct:.1f}%)", flush=True)
