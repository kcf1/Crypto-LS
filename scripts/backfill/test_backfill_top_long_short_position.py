"""
Test script: Backfill Top Long/Short Position for first 10 symbols only.
Quick test to verify collection works before running full backfill.

Run from project root: python scripts/backfill/test_backfill_top_long_short_position.py
"""
...
def fetch_with_retry(collector, symbol, period, *, start_time=None, end_time=None, limit=500):
    """Call collector.fetch_top_long_short_position with retry logic."""
    for attempt in range(MAX_429_RETRIES):
        try:
            rows = collector.fetch_top_long_short_position(
...
def main():
    ...
    logger.info("TEST: Top Long/Short Position backfill for %s symbols, period=%s, last %s days", len(symbols), PERIOD, DAYS_BACK)
    ...
    for i, symbol in enumerate(tqdm(symbols, desc="Symbols")):
        ...
        while current_end_time > start_time_ms:
            ...
            if filtered_rows:
                storage.write_top_long_short_position(symbol, PERIOD, filtered_rows)
