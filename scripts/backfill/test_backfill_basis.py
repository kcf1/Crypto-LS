"""
Test script: Backfill Basis for first 10 symbols only.
Quick test to verify basis collection works before running full backfill.

Run from project root: python scripts/backfill/test_backfill_basis.py
"""
...
def fetch_basis_with_retry(collector, symbol, period, *, start_time=None, end_time=None, limit=500):
    """Call collector.fetch_basis with retry logic."""
    for attempt in range(MAX_429_RETRIES):
        try:
            rows = collector.fetch_basis(symbol=symbol, period=period, start_time=start_time, end_time=end_time, limit=limit)
...
def main():
    ...
    logger.info("TEST: Basis backfill for %s symbols, period=%s, last %s days", len(symbols), PERIOD, DAYS_BACK)
    ...
    for i, symbol in enumerate(tqdm(symbols, desc="Symbols")):
        ...
        while current_end_time > start_time_ms:
            rows = fetch_basis_with_retry(collector, symbol, PERIOD, start_time=start_time_ms, end_time=current_end_time, limit=LIMIT)
            if not rows:
                break
