"""
Test script: Backfill Global Long/Short Account for first 10 symbols only.
Quick test to verify collection works before running full backfill.

Run from project root: python scripts/backfill/test_backfill_global_long_short_account.py
"""
...
def fetch_with_retry(collector, symbol, period, *, start_time=None, end_time=None, limit=500):
    """Call collector.fetch_global_long_short_account with retry logic."""
...
def main():
    ...
    logger.info("TEST: Global Long/Short Account backfill for %s symbols, period=%s, last %s days", len(symbols), PERIOD, DAYS_BACK)
