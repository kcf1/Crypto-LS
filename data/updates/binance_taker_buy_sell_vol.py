"""
Binance Taker Buy/Sell Volume incremental update task.
...
def _fetch_taker_buy_sell_vol_with_retry(
    collector: Collector,
    symbol: str,
    period: str,
    *,
    ...
    """Call collector.fetch_taker_buy_sell_vol; on 429, sleep Retry-After and retry."""
    for attempt in range(MAX_429_RETRIES):
        try:
            return collector.fetch_taker_buy_sell_vol(
                symbol=symbol,
                period=period,
...
def run() -> None:
    """One-shot Binance Taker Buy/Sell Volume incremental update (tail refresh + new records)."""
    ...
    for symbol in settings.symbols:
        latest = storage.get_latest_taker_buy_sell_vol_time(symbol, PERIOD)
        if latest is None:
            logger.debug("Skip %s %s: no stored taker_buy_sell_vol data", symbol, PERIOD)
            continue
        ...
        try:
            rows = _fetch_taker_buy_sell_vol_with_retry(
                collector,
                symbol,
                PERIOD,
            ...
            storage.write_taker_buy_sell_vol(symbol, PERIOD, rows)
            logger.info("%s %s: wrote %s taker_buy_sell_vol records (tail refresh + new)", symbol, PERIOD, len(rows))
        except Exception as e:
            logger.warning("%s %s taker_buy_sell_vol incremental update failed: %s", symbol, PERIOD, e)
