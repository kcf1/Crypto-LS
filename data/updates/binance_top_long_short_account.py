"""
Binance Top Trader Long/Short Account Ratio incremental update task.
...
def _fetch_top_long_short_account_with_retry(
    collector: Collector,
    symbol: str,
    period: str,
    *,
    ...
    """Call collector.fetch_top_long_short_account; on 429, sleep Retry-After and retry."""
    for attempt in range(MAX_429_RETRIES):
        try:
            return collector.fetch_top_long_short_account(
                symbol=symbol,
                period=period,
...
def run() -> None:
    """One-shot Binance Top Trader Long/Short Account incremental update (tail refresh + new records)."""
    ...
    for symbol in settings.symbols:
        latest = storage.get_latest_top_long_short_account_time(symbol, PERIOD)
        if latest is None:
            logger.debug("Skip %s %s: no stored top_long_short_account data", symbol, PERIOD)
            continue
        ...
        try:
            rows = _fetch_top_long_short_account_with_retry(
                collector,
                symbol,
                PERIOD,
            ...
            storage.write_top_long_short_account(symbol, PERIOD, rows)
            logger.info("%s %s: wrote %s top_long_short_account records (tail refresh + new)", symbol, PERIOD, len(rows))
        except Exception as e:
            logger.warning("%s %s top_long_short_account incremental update failed: %s", symbol, PERIOD, e)
